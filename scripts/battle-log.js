...
class Log {
  static PART = Object.freeze({
    ...
    ward: 'ward',
    heal: 'Heal',
    // Healing not yet implemented, but added for future use
    die: 'Die',
  ...
  static getLogData(characterIdxs = null) {
    ...
    const logs = characters.filter(character => character.log.length > 0).filter((c, index) => characterIdxs == null || characterIdxs.includes(index)).map(character => character.log.map(logJSON => new Log(logJSON, character.name))).flat();
    logs.sort((log1, log2) => log2.getTimeStamp() - log1.getTimeStamp());
    return logs.map(log => log.toTableRowData());
  }
  static getLoggedCharacters() {
    const characters = [...DataManager.getCharacters(), ...DataManager.graveyard];
    return characters.filter(character => character.log.length > 0).map((character, index) => ({
      text: character.name,
      value: index
    }));
  }
