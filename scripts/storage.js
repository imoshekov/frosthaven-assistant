...
const DataManager = {
  ...
  loadFile: async function () {
    ...
    const level = parseInt(document.getElementById('scenario-level').value) || parseInt(document.getElementById('scenario-level').placeholder) || 1;
    if (!scenarioNumber) {
  ...
  processScenarioData: function (data, level = 1) {
    data.rooms[0].monster.forEach(monster => {
      const newCreature = {
        ...
        standee: 'X',
        level: level,
        isElite: (monster === null || monster === void 0 ? void 0 : monster.player4) === 'elite'
      };
