...
const WebSocketHandler = {
  ...
  connect: function () {
    this.ws = new WebSocket(window.location.hostname.includes('github.io') ? 'wss://frosthaven-assistant.onrender.com' : 'ws://localhost:8080');
    this.ws.onopen = () => {
    ...
    this.ws.onmessage = event => {
      ...
      if ((data === null || data === void 0 ? void 0 : data.originatingClientId) === this.clientId) {
        return;
      }
      switch (data.type) {
  ...
  keepServerAlive: function () {
    setInterval(() => {
      fetch('https://frosthaven-assistant.onrender.com/ping').then(response => {
        if (!response.ok) {
  ...
  sendGraveyardUpdate: function () {
    this.sendUpdateMessage('graveyard-update', {
      graveyard: DataManager.graveyard
    });
  },
  sendRoundNumber: function (roundNumberValue) {
    this.sendUpdateMessage('round-update', {
      roundNumber: roundNumberValue
    });
  },
  sendElementState: function (elementId, elementState) {
    this.sendUpdateMessage('element-update', {
      elementId: elementId,
  ...
  requestServerState: function (sessionId) {
    this.ws.send(JSON.stringify({
      type: 'request-latest-state',
      sessionId: sessionId
    }));
  },
  sendMonsterAdded: function (monster) {
    this.sendUpdateMessage('add-monster', {
      monster: monster
    });
  },
  sendInitiativeReset: function (value = 0) {
    this.sendUpdateMessage('initiative-reset', {
      value: value
    });
  },
